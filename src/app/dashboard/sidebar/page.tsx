...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col m-2 box-border h-fit md:h-[98vh] md:bg-custom-green text-white rounded-xl  md:w-[350px]">
      {/* Dashboard Name */}
 ...
 <Button onClick={handleLogout} variant="ghost" 
           className={cn(
            "flex p-0 px-0 md:hidden bg-gray-50 h-auto rounded-sm mt-2 lg:flex-row items-center lg:justify-start text-2xl gap-2 w-full border-2 border-white/20 transition-all duration-300 ease-in-out transform hover:bg-emerald-50 hover:scale-105",
            pathname?.startsWith("/") ? "bg-emerald-50 text-custom-green" : ""
          )}>
...
//       {/* Navigation Menu */}
//       <div className="flex flex-col md:h-full justify-between">
//         <nav className="flex flex-row md:flex-col items-center md:items-start gap-4 md:gap-3 pt-4">
//           {menuItems.map((item) => (
//             <Link key={item.href} href={item.href} className="w-full px-[2px]">
//               <Button
//                 variant={pathname?.startsWith(item.href) ? "secondary" : "ghost"}
...
//             variant="ghost" 
//             className="flex md:px-5 lg:flex-row h-auto transition-all duration-300 ease-in-out transform hover:bg-white hover:text-custom-green hover:scale-105 justify-start gap-2 w-full md:py-2 text-base border-white/20"
//           >
//             <Power className="h-5 w-5" />
